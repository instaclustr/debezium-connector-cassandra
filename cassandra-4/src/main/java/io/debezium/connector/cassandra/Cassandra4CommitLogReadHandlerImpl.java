...
public class Cassandra4CommitLogReadHandlerImpl implements CommitLogReadHandler {
    ...
    private void process(PartitionUpdate pu, OffsetPosition offsetPosition, KeyspaceTable keyspaceTable) {
        ...
        LOGGER.info("IN PROCESS 1");

        if (!PartitionType.isValid(partitionType)) {
            LOGGER.info("IN PROCESS 2");
    ...
    private void populateClusteringColumns(RowData after, Row row, PartitionUpdate pu) {
        ImmutableList<org.apache.cassandra.schema.ColumnMetadata> columns = pu.metadata().clusteringColumns();
        for (int i = 0; i < columns.size(); i++) {
            org.apache.cassandra.schema.ColumnMetadata metadata = columns.get(i);
            String name = metadata.name.toString();
            AbstractType<?> type = metadata.type;
            try {
                Object value = CassandraTypeDeserializer.deserialize(type, row.getCell(metadata).buffer());
                CellData cellData = new CellData(metadata.name.toString(), value, null, CellData.ColumnType.CLUSTERING);
                after.addCell(cellData);
            }
            catch (Exception e) {
                throw new DebeziumException(String.format("Failed to populate Column %s with Type %s of Table %s in KeySpace %s.",
                        name, type, metadata.cfName, pu.metadata().keyspace), e);
            }
        }
    }
