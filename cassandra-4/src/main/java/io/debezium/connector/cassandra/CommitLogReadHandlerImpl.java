...
public class CommitLogReadHandlerImpl implements CommitLogReadHandler {
    ...
    private final AtomicReference<OffsetPosition> latestOffsetPosition = new AtomicReference<>(null);
    ...
    public OffsetPosition getLatestOffsetPosition() {
        return latestOffsetPosition.get();
    }

    @Override
    public void handleMutation(Mutation mutation, int size, int entryLocation, CommitLogDescriptor descriptor) {
        LOGGER.info(String.format("Processing mutation with entry location: %d", entryLocation));
        ...
        for (PartitionUpdate pu : mutation.getPartitionUpdates()) {
            ...
            try {
                ...
                latestOffsetPosition.set(offsetPosition);
            }
        }
